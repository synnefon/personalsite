...
const noteSubCell = (cell, highlightCell, selectedVal, n) => {
  ...
  const shouldDisplay = cell.val === ".";
  return (
    <div key={key} className={`note-sub-cell ${shouldHighlightText && "text-highlight"}`}>
      {<div className={`${cell.notes.includes(n) && shouldDisplay ? "" : "invisible-num"}`}>{n}</div>}
    </div>
  );
};
...
const DisplayableCell = ({cell, highlightCell, selectedVal}) => {
  ...
  return <div className='note-cell'>
    <div className='note-row'>{["1", "2", "3"].map(n => makeSubCell(n))}</div>
    <div className='note-row'>{["4", "5", "6"].map(n => makeSubCell(n))}</div>
    <div className='note-row'>{["7", "8", "9"].map(n => makeSubCell(n))}</div>
  </div>;
};
